...
const generateValidator = <T>(
    ...
    validationFunctions?: ((value: T) => Promise<ValidationModels.ValidationType>)[]
): (value: unknown) => Promise<ValidationModels.ValidationType> => {
    return async (value: unknown): Promise<ValidationModels.ValidationType> => {
        const requiredValidation = validateRequired(key, value);
        if (!required && !requiredValidation.success) {
            return new ValidationModels.ValidationSuccessModel(null);
        }
        ...
        if (validationFunctions) {
            for (const func of validationFunctions) {
                const funcValidation = await func(typedValue);
        ...
        return new ValidationModels.ValidationSuccessModel(typedValue);
    };
};  
