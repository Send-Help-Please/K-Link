...
import { PASSWORD_NO_LOWERCASE, PASSWORD_NO_NUMBER, PASSWORD_NO_SPECIAL_CHARACTER, PASSWORD_NO_UPPERCASE, PASSWORD_NO_MATCH } from "../constants/PasswordErrors";
...
const validatePassword = async (password: unknown): Promise<ValidationModels.ValidationType> => {
    if(typeof password !== "string"){
        return new ValidationModels.ValidationErrorModel({ errorMessages: [notString(PASSWORD_VERBOSE)]});
    }

    if(validator.isAscii(password) === false){
        return new ValidationModels.ValidationErrorModel({ errorMessages: [notAscii(PASSWORD_VERBOSE)]});
    }
    ...
    if(errors.length > 0){
        return new ValidationModels.ValidationErrorModel({ errorMessages: errors });
    }

    return new ValidationModels.ValidationSuccessModel({ data: password });
}
